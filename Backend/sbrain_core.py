"""
S-Brain Core v4 - Full S-Series Aerospace Standards Brain
Fully uses all 34 S1000D XSD files + ready for S2000M / S3000L
"""
...
STANDARD_PROFILES = {
    "S1000D": {
        ...
        "key_entities": ["Data Module", "IPL", "Illustrated Parts List", "Data Module Code", "DMC", "BREX", "CSDB",
                         "Publication Module", "Information Set", "Applicability", "Effectivity", "Common Source Database",
                         "Business Rules", "Data Module Requirement List", "DMRL", "Schema", "XML", "SGML"],
        "key_sections": ["scope", "definitions", "data module", "ipl", "procedure", "description", "maintenance",
                         "repair", "illustrated parts", "applicability", "brex", "publication", "catalog", "supply", "figure"],
        "schema_entities": ["DataModule", "IPLItem", "Task", "Part", "Tool", "Figure"],
        "skip_title_patterns": [r"^\[PREAMBLE\]", r"\.{5,}", r"^\s*\d+\s*$"],
        "skip_rules_in": ["[preamble]", "table of contents", "copyright", "user agreement", "license to use", "license",
                          "intellectual property", "no modifications"],
    },
    "S2000M": {
        "description": "Materiel Management Standard (Logistics)",
        "key_entities": ["Provisioning", "Spare Part", "Supply", "Procurement", "Vendor", "Supplier", "Stock", "Inventory",
                         "Lead Time", "Unit of Issue", "NSN", "Part Number", "Source of Supply", "RSPL"],
        "key_sections": ["provisioning", "supply", "logistics", "materiel", "procurement", "inventory", "stock", "vendor"],
    ...
    "S3000L": {
        ...
        "key_entities": ["LSA", "Logistics Support Analysis", "FMEA", "FMECA", "Failure Mode", "Maintenance Task",
                         "LCC", "Life Cycle Cost", "MTBF", "MTTR", "Reliability", "Maintainability", "LORA", "LSAR"],
        "key_sections": ["lsa", "maintenance", "analysis", "failure mode", "reliability", "maintainability", "task"],
...
# ─────────────────────────────────────────────
# PDF PARSER LAYER (unchanged - good)
# ─────────────────────────────────────────────
class PDFParser:
    ...
    def get_page_text(self, page_num: int) -> dict:
        ...
        return {"text": page.get_text("text"), "blocks": page.get_text("blocks"), "page_num": page_num}
        ...
        try:
            for tbl in page.find_tables():
                ...
                if df and len(df) > 1:
                    ...
                    rows = [{headers[j] if j < len(headers) else f"col_{j}": str(v).strip() if v else "" 
                             for j, v in enumerate(row)} for row in df[1:]]
                    tables.append({"page": page_num, "headers": headers, "rows": rows[:100], "row_count": len(df)-1, "source": "fitz"})
        except Exception:
            pass
        return tables
    ...
    def extract_tables_plumber(self, page_num: int) -> list:
        ...
        try:
            ...
            settings = {"vertical_strategy": "lines", "horizontal_strategy": "lines", "snap_tolerance": 3, "join_tolerance": 3}
            raw = page.extract_tables(settings) or []
            for tbl in raw:
                if not tbl or len(tbl) < 2: continue
                headers = [str(h or "").strip() or f"col_{i}" for i, h in enumerate(tbl[0])]
                rows = [{headers[j] if j < len(headers) else f"col_{j}": str(v or "").strip() for j, v in enumerate(row)} 
                        for row in tbl[1:]]
                if any(any(v for v in r.values()) for r in rows):
                    tables.append({"page": page_num, "headers": headers, "rows": rows[:100], "row_count": len(tbl)-1, "source": "pdfplumber"})
        except Exception:
            pass
        return tables
    ...
    def close(self):
        self.doc.close()
        if self._plumber: self._plumber.close()


# ─────────────────────────────────────────────
# SECTION SEGMENTER (unchanged)
# ─────────────────────────────────────────────
class SectionSegmenter:
    ...
    def _heading_level(self, line: str, font_size: float = None, bold: bool = False) -> Optional[int]:
        ...
        if m: return min(m.group(1).count(".") + 1, 6)
        m2 = self.NUMBERED_SEC.match(line)
        if m2: return min(line.split()[0].count(".") + 1, 6)
        if font_size:
            if font_size >= self.font_sizes.get("h1", 16) - 0.5: return 1
            if font_size >= self.font_sizes.get("h2", 14) - 0.5: return 2
            if bold and font_size >= self.font_sizes.get("h3", 12) - 0.5: return 3
        return None

    def segment(self, pages_data: list) -> list:
        sections = []
        current = Section(title="[PREAMBLE]", content="", level=0, page_start=0, page_end=0, section_type="body")
        for page_data in pages_data:
            page_num = page_data["page_num"]
            for line in page_data["text"].split("\n"):
                line_s = line.strip()
                if not line_s: continue
                if self.TOC_DOTS.search(line_s): continue
                ...
                if level is not None:
                    ...
                    current = Section(title=line_s[:200], content="", level=level, page_start=page_num, page_end=page_num,
                                      section_type="heading", is_toc=is_toc, is_changelog=is_changelog)
                else:
                    if self.CPF_LINE.search(line_s): current.is_changelog = True
...
# ─────────────────────────────────────────────
# XSD PARSER + MULTI STANDARD SUPPORT
# ─────────────────────────────────────────────
class MultiStandardXSDParser:
    def __init__(self):
        ...
        # Per-standard critical structures (no longer S1000D-only)
        self.critical_map = {
            "S1000D": ["ipd.xsd", "wrngflds.xsd", "descript.xsd", "proced.xsd", "brex.xsd", "pm.xsd", "fault.xsd"],
            "S2000M": ["mmDataset.xsd", "valid_values"],          # provisioning, parts, NSN
            "S3000L": ["lsaDataset.xsd", "valid_values"]          # LSA, tasks, failure modes
        }

    def load_standard(self, standard: str, xsd_dir: str):
        print(f"  Loading XSDs + XMI for {standard} from {xsd_dir}...")

        schemas, hierarchy = parse_multi_standard_xsd_dir(xsd_dir, standard)

        # XMI parsing
        xmi_file = next(Path(xsd_dir).glob("**/*data_model*.xmi"), None)
        xmi_count = 0
        if xmi_file:
            xmi_data = self._parse_xmi(xmi_file, standard)
            self.xmi_models[standard] = xmi_data
            xmi_count = xmi_data.get("total_classes", 0)
        else:
            print(f"    No XMI file found in {xsd_dir}")

        critical_count = sum(1 for s in schemas if s.get("is_critical", False))
        print(f"    → {len(schemas)} XSD elements | {len(hierarchy)} hierarchy edges | "
              f"{critical_count} critical structures | {xmi_count} XMI classes loaded")
        
        self.schemas[standard] = schemas
        self.hierarchy[standard] = hierarchy

    def _parse_xmi(self, xmi_path: Path, standard: str) -> dict:
        """Robust Enterprise Architect (EA) XMI parser for S2000M / S3000L data models"""
        try:
            tree = ET.parse(str(xmi_path))
            root = tree.getroot()

            classes = []
            class_map = {}  # Object_ID → class name (for linking attributes)

            # 1. Extract Classes from t_object table
            ...
            # 2. Extract Attributes from t_attribute table
            for table in root.findall(".//Table"):
            ...
            # 3. Extract Associations / Connectors from t_connector (if present)
            for table in root.findall(".//Table"):
                if table.get("name") != "t_connector":
                    continue
                for row in table.findall("Row"):
                    cols = {col.get("name"): col.get("value") for col in row.findall("Column")}
                    conn_name = cols.get("Name")
                    if conn_name:
                        for c in classes:
                            c["associations"].append(conn_name)

            total_classes = len(classes)
            total_attrs = sum(len(c["attributes"]) for c in classes)
            print(f"    XMI parsed successfully: {total_classes} classes + {total_attrs} attributes from {xmi_path.name}")
...
def parse_s1000d_xsd_dir(xsd_dir: str) -> tuple[list, list]:
    ...
    global_types = {}
    global_groups = {}
    global_attr_groups = defaultdict(list)
    all_roots = []
    ...
    for xsd_file in xsd_files:
        try:
            ...
            for ct in root.findall(f".//{{{XS_NS}}}complexType"):
                if name := ct.get("name"): global_types[name] = ct
            for grp in root.findall(f".//{{{XS_NS}}}group"):
                if name := grp.get("name"): global_groups[name] = grp
            for ag in root.findall(f".//{{{XS_NS}}}attributeGroup"):
                if name := ag.get("name"):
                    attrs = [{"name": a.get("name"), "type": a.get("type","string"), "required": a.get("use","optional") == "required"}
                             for a in ag.findall(f".//{{{XS_NS}}}attribute") if a.get("name")]
    ...
    for fname, root in all_roots:
        for elem in root.findall(f".//{{{XS_NS}}}element"):
            name = elem.get("name")
            if not name or name in seen_elements: continue
            seen_elements.add(name)

            inline_ct = elem.find(f".//{{{XS_NS}}}complexType")
            type_ref = elem.get("type")
            ct = inline_ct if inline_ct is not None else global_types.get(type_ref.split(":")[-1] if type_ref else None)
            ...
            schemas.append({
                "element": name,
                "children": list(dict.fromkeys(c for c in children if c != name))[:40],
                "attributes": attributes[:25],
                "required": elem.get("minOccurs", "0") != "0",
                "source_file": fname,
                "source": "xsd"
            })
            ...
            for child in children:
                if child != name:
                    hierarchy.append({"parent": name, "child": child, "source_file": fname, "source": "xsd"})

    print(f"  XSD Parse Complete: {len(schemas)} elements, {len(hierarchy)} relationships")
    return schemas, hierarchy

def extract_children_from_complex_type(ct, ns):
    children = []
...
def parse_multi_standard_xsd_dir(xsd_dir: str, standard: str) -> tuple[list, list]:
    xsd_dir = Path(xsd_dir)
    xsd_files = list(xsd_dir.glob("**/*.xsd"))
    ...
    # Step 1: collect global complexTypes
    global_types = {}
    ...
    for xsd_file in xsd_files:
        try:
            ...
            for elem in root.findall(f".//{{{XS_NS}}}element"):
                ...
                children = extract_children_from_complex_type(ct, XS_NS)
...
def resolve_children(node):
    ...
    children = []
    for child in node:
        tag = child.tag.replace(f"{{{XS_NS}}}", "")
        if tag == "element":
            ref = child.get("ref") or child.get("name")
            if ref and ":" not in ref: children.append(ref)
        elif tag in ("sequence", "choice", "all"):
            children.extend(resolve_children(child))
        elif tag == "group":
            ref = child.get("ref")
            # global_groups handling can be added if needed
    return children

def resolve_attributes(node):
    if node is None: return []
    attrs = []
...
class CrossStandardMapper:
    """Core mapping between S1000D, S2000M, S3000L"""
    
    MAPPINGS = {
        "part_number": {
            "S1000D": ["partNumberValue", "partNumber", "descrPartNumber"],
            "S2000M": ["partNumber", "nsn", "itemIdentifier"],
            "S3000L": ["partNumber", "productItem"]
        },
        "nsn": {
            "S1000D": ["natoStockNumber"],
            "S2000M": ["nsn", "natoStockNumber"],
            "S3000L": ["nsn"]
        },
        "quantity": {
            "S1000D": ["quantityValue", "quantity"],
            "S2000M": ["quantity", "orderQuantity"],
            "S3000L": ["quantity"]
        },
        "unit_of_measure": {
            "S1000D": ["quantityUnitOfMeasure", "unitOfMeasure"],
            "S2000M": ["unitOfIssue", "uom"],
            "S3000L": ["unitOfMeasure"]
        },
        "description": {
            "S1000D": ["nomenclature", "descr"],
            "S2000M": ["itemName", "description"],
            "S3000L": ["name", "description"]
        }
    }

    def map_entity(self, entity_name: str, from_standard: str, to_standard: str) -> list:
        """Return possible equivalent field names in target standard"""
        canonical = Normalizer().normalize_field_name(entity_name)
        mappings = self.MAPPINGS.get(canonical, {})
        
        source_fields = mappings.get(from_standard, [])
        target_fields = mappings.get(to_standard, [])
        return target_fields if target_fields else [canonical]

# LLM for POC
class StubLLM:
    """
    Zero-download 'LLM' for POC. Just formats the top RAG chunks into a
    readable answer. Replace with Ollama/real model when ready.
    """
...
class SBrainRAG:
    def __init__(self, model_path='all-MiniLM-L6-v2', output_dir="./output"):
        self.model = SentenceTransformer(model_path)
        self.index = None
        self.documents = []
        self.metadata = []
        # Save alongside your JSON outputs
    ...
    def build_index(self, ontology: dict, extracted_results: dict):
        ...
        if not docs:
            print("  ⚠️  RAG: No documents to index — skipping build")
            return
        ...
        print(f"  ✅ RAG built & saved to {self.index_path} ({len(docs)} docs)")
    ...
    def load_index(self):
        if (os.path.exists(self.index_path)
            ...
            print(f"  ✅ RAG loaded from {self.index_path} ({len(self.documents)} docs)")
            return True
        return False
...
class AerospaceExtractorPipeline:
    def __init__(self, output_dir: str = "./output", xsd_dirs: dict = None):
        ...
        # ── LLM: stub for POC, swap in real model later ──
        self.llm = StubLLM()
        # To use Ollama locally (no download needed if already installed):
        # self.llm = OllamaLLM(model="mistral")  # see note below

        self.max_pages    = None
        self.max_sections = None

        # ── RAG: pass output_dir so .faiss/.npy land in ./output/ ──
        ...
        if xsd_dirs:
            if isinstance(xsd_dirs, str):
                try:    xsd_dirs = json.loads(xsd_dirs)
                except: xsd_dirs = {}
    ...
    def process(self, pdf_path: str, standard: str = None) -> DocumentResult:
        ...
        print(f"  Pages: {parser.total_pages} | Dominant fonts: {font_sizes}")

        page_limit = getattr(self, 'max_pages', None) or parser.total_pages
        pages_data = [
        ...
        if getattr(self, 'max_sections', None):
        ...
        for sec in tqdm(sections_content, desc="  Extracting", ncols=80):
            ...
            for k in ("dmc_references", "part_numbers", "nsn_references", "tasks", "failure_modes", "metrics"):
        ...
        critical_count = sum(1 for s in all_schemas if s.get("is_critical", False))
        xmi_count = len(self.xsd_parser.xmi_models.get(standard, {}).get("classes", []))

        print(f"  Schema Status: {len(all_schemas)} elements | {len(hierarchy)} hierarchy edges | "
              f"{critical_count} critical | {xmi_count} XMI classes")
        ...
        for entity in all_entities:
            ...
            if name:
                mapped = mapper.map_entity(name, standard, 
                                         "S2000M" if standard == "S1000D" else 
                                         "S1000D" if standard == "S2000M" else "S3000L")
                if mapped and mapped != [name]:
                    entity["cross_mapped_to"] = mapped
                    mapped_entities.append({"source": name, "from": standard, "to": mapped})

        print(f"  Cross-mapping: {len(mapped_entities)} entities linked across standards")
        ...
        extra_meta = {
            ...
            "cross_mapped_count": len(mapped_entities),
            "hierarchy": hierarchy[:500]  # limit for size
        }
        ...
        print(f"  ✅ Extraction done — RAG will be built after all documents are processed")
        parser.close()

        print(f"\n  ✅ {standard} Complete | Entities: {len(all_entities)} | Rules: {len(all_rules)} | "
              f"Critical: {critical_count} | Cross-mapped: {len(mapped_entities)}")
        return doc_result
    
    def _build_temp_ontology(self):
    ...
    def save_all(self):
        ...
        # === Build RAG ONCE across ALL standards ===
        print("\n  🔨 Building unified RAG index across all standards...")
        ...
        print(f"  ✅ Unified RAG ready — {len(self.rag.documents)} docs across {list(self.all_results.keys())}")
    ...
    def generate_answer(self, question: str, results: list) -> str:
        ...
        prompt = f"""
        You are an aerospace standards expert.

        STRICT RULES:
        - Answer ONLY using the provided context
        - If unsure, say "Not found in standards"
        - Compare standards clearly if multiple are present
...
def smart_filter(results, question):
    q = question.lower()

    if "part number" in q:
        return [r for r in results if "part" in r["text"].lower()]

    if "schema" in q:
        return [r for r in results if r["meta"]["type"] == "schema"]
...
def interactive_query(pipeline):
    ...
    while True:
        ...
        results = pipeline.rag.query(q)
        results = smart_filter(results, q)
        if not results:
            return "No relevant information found in standards."
        
        # 🔥 ADD THIS BLOCK HERE
        top_results = sorted(results, key=lambda x: x["score"])[:5]

        context = "\n\n".join([
            f"[{r['meta']['standard']} | {r['meta']['type']}]\n{r['text']}"
            for r in top_results
        ])
        answer = pipeline.generate_answer(q, results)

        print("\n🧠 Final Answer:\n")
        print(answer)

        # 👇 Keep this if you still want raw debug results
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="S-Brain v4.2")
    parser.add_argument("pdfs", nargs="+")
    parser.add_argument("--standard", nargs="+")
    parser.add_argument("--output", default="./output")
    parser.add_argument("--xsd-dirs", type=str, default=None, 
                        help='JSON dict like {"S1000D":"Inputs/S1000D"}')
